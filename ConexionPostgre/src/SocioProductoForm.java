...
public class SocioProductoForm extends JFrame {
    ...
    public SocioProductoForm() {
        ...
        modelo = new DefaultTableModel(new String[]{"ID", "Socio", "Producto", "Cuenta", "Fecha", "Saldo", "Estado"}, 0) {
    ...
    void listarDatos() {
        ...
        try (Connection conn = ConexionDB.conectar()) {
            String sql = "SELECT sp.id_socpro, s.soc_nom, p.pro_iden, sp.num_cuenta, sp.fec_apertura, sp.saldo, sp.socpro_estado " +
                         "FROM socioproducto sp JOIN socio s ON sp.id_socio=s.soc_cod " +
                         "JOIN producto p ON sp.id_producto=p.pro_cod";
            ResultSet rs = conn.createStatement().executeQuery(sql);
            while (rs.next()) {
                modelo.addRow(new Object[]{rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getDate(5), rs.getBigDecimal(6), rs.getString(7)});
            }
        } catch (SQLException e) {
